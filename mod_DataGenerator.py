...
def main_DataGenerator(filepath, role, percentile,patchsize,channels,ch1,ch2,ch3,l_layer,u_layer,biosample,datasize,mode,alpha,Norm,Brightness):
    ...
    if role == "train_test":
        ...
        # El notebook esta preparado para tratar archivos con 3 imágenes (canales). Sin embargo mis archivos lif solo tienen 1 por serie.
        # Una posible solución es que se tomen las series como canales. Sin embargo, .get_frame se empeña en coger c como 
        # int, no le sirve que le de los canales ya "sacados".

        selected_channels = [ch1,ch2,ch3] # Uso los canales especificados en el notebook

        if filepath.endswith('.lif'): # Ejecutamos este codigo si el archivo es .lif
            m_idx = 3
            
            lif = LifFile(filepath)   # Leemos el .lif
            for i in selected_channels: # Vamos canal por canal elegido
                lim = lif.get_image(i)  # Definimos como canal la serie de número ch1, ch2 y ch3
                n_tiles = lim.dims[m_idx]
                for m in range(n_tiles):
                    data = lim.get_frame(z=0, c=lim, t=0, m=m)    # ERROR TypeError: int() argument must be a string, a bytes-like object or a number, not 'LifImage'
